...
def save_image(image, filepath, extension, quality_jpeg_or_webp, lossless_webp, optimize_png, a111_params, prompt,
               extra_pnginfo, embed_workflow):
    # 若文件夹不存在, 则创建文件夹
    dirname = os.path.dirname(filepath)
    if not os.path.exists(dirname):
        print("Directory doesn't exist, creating:", dirname)
        os.makedirs(dirname)
    if extension == 'png':
        metadata = PngInfo()
        ...
        if embed_workflow:
            if extra_pnginfo is not None:
                pnginfo_json = {piexif.ImageIFD.Make - i: f"{k}:{json.dumps(v, separators=(',', ':'))}" for i, (k, v) in
                                enumerate(extra_pnginfo.items())}
            if prompt is not None:
        ...
        def get_exif_bytes() -> bytes:
            return piexif.dump(({
                                   ...
                                   "Exif": {
                                       piexif.ExifIFD.UserComment: piexif.helper.UserComment.dump(a111_params,
                                                                                                  encoding="unicode")
                                   },
                               })
